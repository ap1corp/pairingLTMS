 ...
 */
(function($) {
    var urll = "<?php echo _BASE_URL;?>tool/model/upload_sertificate.php";
    $.fn.pekeUpload = function(options) {
        var defaults = {onSubmit: false, btnText: "Browse files...", url: urll, theme: "custom", field: "file", data: null, multi: true, showFilename: true, showPercent: true, showErrorAlerts: true, allowedExtensions: "xls|xlsx", invalidExtError: "Invalid File Type", maxSize: 0, sizeError: "Size of the file is greather than allowed", onFileError: function(file, error) {
        ...
        function UploadFile() {
            //if ($("#period").val() && $("#range-start").val() && $("#range-end").val()) {
                $("#ov-dialog").hide();
                var error = true;
                if (options.theme == "bootstrap") {
                ...
                $.ajax({url: options.url, type: 'POST', data: formData, success: function(data) {
                        var percent = 100;
                ...
                return error
            //} else {
            //    showMessage("<b>[Month to Compare]</b> and <b>[Date Range]</b> are mandatory !");
            //}
        }
